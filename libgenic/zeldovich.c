...
void displacement_fields(PetaPM * pm, enum TransferType Type, struct ic_part_data * dispICP, const int NumPart, Cosmology * CP, const struct genic_config GenicConfig) {
    ...
    double maxdisp = 0, maxvel = 0, avg_disp=0;
    ...
    for(i = 0; i < NumPart; i++)
    {
        ...
        for(k = 0; k < 3; k++)
        {
            ...
            //avg_disp += curICP[i].Disp[k]; //MM added this; can take it out
    ...
    //printf("displacement of particle type %d is %f\n", Type, avg_disp/NumPart);
