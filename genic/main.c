...
int main(int argc, char **argv)
{
  ...
  if(NumPartCDM > 0) {
    ...
    //boost only CDM and leave baryons fixed (as this is better numerically)
    int i;
    for(i = 0; i < NumPartCDM; i++) {
      ICP[i].Vel[0] += CP.Stream_Velocity; //All2.units.UnitVelocity_in_cm_per_s; //put in correct units
    }
